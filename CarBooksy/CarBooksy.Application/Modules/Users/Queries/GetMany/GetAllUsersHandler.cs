...
public class GetAllUsersHandler(ApplicationDbContext context) : IRequestHandler<GetAllUsersQuery, IQueryable<User>>
{
    public Task<IQueryable<User>> Handle(GetAllUsersQuery request, CancellationToken cancellationToken)
        => Task.FromResult(context.Users.AsQueryable());
}